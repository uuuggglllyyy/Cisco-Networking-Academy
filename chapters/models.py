...
class Module(models.Model):
    ...
    def completed_chapters_count(self):
        """Количество завершенных глав в модуле для текущего пользователя"""
        if not hasattr(self, '_completed_chapters_count'):
            # Вам нужно передать user в контекст или вычислить это в view
            self._completed_chapters_count = 0
        return self._completed_chapters_count

    @property
    def is_completed(self):
        """Проверка, завершен ли модуль полностью"""
        return self.completed_chapters_count == self.chapters.count()
...
class Chapter(models.Model):
    ...
    @property
    def is_completed(self):
        """Проверка, завершена ли глава полностью"""
...
class Section(models.Model):
    ...
    video_file = models.FileField(
        ...
        help_text="Загрузите видеофайл (MP4, WebM, OGG)"
    )
    ...
    # СВОЙСТВО ДЛЯ ПРОВЕРКИ НАЛИЧИЯ ВИДЕО
    @property
    def has_video(self):
